...
+function($, window){ 'use strict';
	...
	sidebar.init = function() {
		...
		sidebar.listenForEvents = function() {
			...
			self.initScroll();
			self.toggleScroll();
			$sidebar.find('.has-submenu.open > .submenu').show();
		...
		$body.on('mouseenter mouseleave', '.folded:not(.open) .has-submenu', function(e){
			...
			$body.on('click', '.submenu-toggle', function(e){
				...
				var $siblings = $item.siblings('.has-submenu');
				...
				$siblings.children('.submenu').stop(true, true).slideUp(200);
	...
	sidebar.toggleScroll = function(){
		...
		if($body.hasClass("sb-folded")){
			...
			$scrollContainer.css("overflow","scroll").parent().css("overflow","scroll");
